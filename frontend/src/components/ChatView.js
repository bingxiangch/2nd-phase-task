...
export const ChatView = () => {
  const { messages, loading, error, addMessage, setLoading, setError, gptmessages,addgptMessage } = useChat();
  const [newMessage, setNewMessage] = useState('');

  const handleSendMessage = async () => {
    if (newMessage.trim() !== '') {
      addMessage({ text: newMessage, sender: 'user' });
      addgptMessage({ text: newMessage, sender: 'user' });
      setNewMessage('');
      setLoading(true);
      ...
      try {
        const response = await api.post(`${BASE_URL}chat_gpt`, { prompt: newMessage });
        addgptMessage({ text: response.data.response, sender: 'bot' });
      } catch (error) {
        handleApiError(error);
      } finally {
        setLoading(false);
    ...
    <main className="bg-slate-50 p-6 sm:p-10 flex-auto">
      ...
      <div>
        ...
        <div style={{ height: '300px', overflowY: 'auto', border: '1px solid #ccc', padding: '10px' }}>
          {gptmessages.map((gptmessages, index) => (
            <div key={index} style={{ marginBottom: '10px', textAlign: gptmessages.sender === 'user' ? 'right' : 'left' }}>
              <strong>{gptmessages.sender === 'user' ? 'You' : 'Bot'}:</strong> 
              <div dangerouslySetInnerHTML={{ __html: gptmessages.text.replace(/(?:\r\n|\r)(?![\n*])/g, '<br />').replace(/<ol>/g, '<ol class="list-decimal" style="padding-left: 20px;">') }} />
            </div>
          ))}
        </div>
        <div style={{ height: '300px', overflowY: 'auto', border: '1px solid #ccc', padding: '10px' }}>
